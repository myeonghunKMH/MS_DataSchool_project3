...
// Key Vault 로더 및 설정 객체 import
const { config, loadConfig } = require('./config.js');

// Keycloak 및 DB 모듈은 설정이 로드된 후 초기화되어야 하므로, require 문을 startServer 함수 내부로 이동하거나,
// 각 모듈이 동적으로 설정을 참조하도록 수정해야 합니다. 여기서는 간단하게 동적 참조로 가정합니다.
...
async function startServer() {
  // 애플리케이션 시작 전, Key Vault에서 모든 설정을 로드합니다.
  await loadConfig();
  ...
  app.post('/api/user/request-deletion', keycloak.protect(), async (req, res) => {
      try {
          ...
          const { data: federatedIdentities } = await axios.get(
              `${config.KEYCLOAK_SERVER_URL}/admin/realms/${config.KEYCLOAK_REALM}/users/${user.keycloak_uuid}/federated-identity`,
...
`close`,
  ...
  async function getKeycloakAdminToken() {
      ...
      params.append('client_id', config.KEYCLOAK_ADMIN_CLIENT_ID);
      params.append('client_secret', config.KEYCLOAK_ADMIN_CLIENT_SECRET);
      params.append('grant_type', 'client_credentials');

      try {
          const { data } = await axios.post(
              `${config.KEYCLOAK_SERVER_URL}/realms/${config.KEYCLOAK_REALM}/protocol/openid-connect/token`,
              params
          );
  ...
  async function deleteUserFromKeycloak(keycloak_uuid, adminToken) {
      try {
          await axios.delete(
              `${config.KEYCLOAK_SERVER_URL}/admin/realms/${config.KEYCLOAK_REALM}/users/${keycloak_uuid}`,
              {
                  headers: { Authorization: `Bearer ${adminToken}` },
              }
          );
